...
// jason 객체 데이터 배열을 forEach 메서드로 순회 - 배열 요소 각각을 분리 - 분리된 각각의 데이터는 item 파라미터에 전달
...
// 라이트 박스 활성화 함수 - 클릭한 이미지를 thisElmt 파라미터로 전달
const activateLightBox = (thisElmt) => {
  ...
  // 라이트 박스가 활성화 될 때 클릭된 이미지 요소의 src 속성값을 읽어 라이트 박스 이미지 src 속성값으로 전달
...
// 이미지 요소들을 순회하여 각각을 클릭했을 때 activLightBox 함수 실행 : 클릭한 대상 요소를 파라미터로 전달
images.forEach((img) => {
...
// 이미지 요소 배열을 순회한다. 첫번째 파라미터는 이미지 요소 배열, 두번째 파라미터는 boolean 값 - true일 경우 ? 뒤에 :을 기준으로 앞에 로직이 적용되고, fales일 경우 뒤에 로직이 적용된다.
...
// filter() : https://velog.io/@haleyjun/JavaScript-filter-%EC%82%AC%EC9A%A9%EB%B2%95
// method : 어떠한 객체의 멤버가 함수일 경우 "메서드"라고 부른다.
// Array.from() : 유사배열(NodeList, HTMLCollection)을 정식 배열로 전환
// SetTimeout() : https://www.freecodecamp.org/korean/news/how-to-set-a-timer-in-javascript/

// 첫 번째 파라미터 : 함수 - 실행할 코드, 두 번째 파라미터 : 숫자 - 시간(m/s) == 밀리초 단위 시간 이후 함수 실행
// setTimeout(function () {
//   alert('Hello World');
// }, 1000);
...
// 2초 간격으로 메시지를 보여줌
// 변수에 값을 저장하게 되면, 변수를 이용하여 일정 시간 이후 실행으 정지시킬 수 있다. (clearInterval(timerId))
...
// const arr = [1, 4, 3, 5, 6, 7, 9, 4, 6, 4, 7];
// let result = arr.filter((value) => value === 4);
// console.log(result);
